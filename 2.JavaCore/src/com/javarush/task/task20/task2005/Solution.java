...
public class Solution {
    public static void main(String[] args) {
        ...
        try {
            ...
            Human ivanov = new Human("Ivanov", null, null);//new Asset("home"), new Asset("car"));
    ...
    public static class Human {
        ...
        public void save(OutputStream outputStream) throws Exception {
            ...
            printWriter.println(this.name);// == null ? "Имени нет" : this.name);
            if (this.assets.size() > 0) {
                for (Asset current : this.assets)
                    printWriter.println(current == null ? "null" : current.getName());
            }
            printWriter.close();
        }
        ...
        public void load(InputStream inputStream) throws Exception {
            ...
            this.name = reader.readLine();//NameInFile.equals("Имени нет") ? null : NameInFile;
            String assetName;
            while ((assetName = reader.readLine()) != null)
                this.assets.add(assetName.equals("null") ? null : new Asset(assetName));
            reader.close();
        }
    }
}
